...
public class CSVBuilder extends AbstractCSVView implements WebAppConstants {
	...
	@Override
	protected void buildCsvDocument(ICsvListWriter csvWriter, Map<String, Object> model) throws Exception {
	...
	private void writeDetailedMandateCSVReport(ICsvListWriter csvWriter, Map<String, Object> model) throws Exception {
		...
		String[] mandateReportHeaders = new String[] { "Mandate Code", "Subscriber Mandate Code", "Payer Name",
				"Biller", "Biller's Bank", "Product", "Amount", "Customer's Email", "Customer's PhoneNumber",
				"Customer's Bank", "Account Number", "Account Name", "Mandate Effective Date", "Mandate Expiry Date",
				"Channel", "Status", "Debit Frequency", "Date Created", "Created By", "Date Verified", "Verified By",
				"Date Approved", "Approved By", "Date Authorized", "Authorized By",
		...
		mandates.stream().forEach(mandate -> {
			try {
				final String[] obj = new String[] { 
						...
						mandate.getPayerName(), mandate.getProduct().getBiller().getCompany().getName(),
						mandate.getProduct().getBiller().getBank().getBankName(), mandate.getProduct().getName(),
						mandate.getAmount().toPlainString(), mandate.getEmail(), mandate.getPhoneNumber(),
						mandate.getBank().getBankName(), mandate.getAccountNumber(), mandate.getAccountName(),
						mandate.getStartDate().toString(), mandate.getEndDate().toString(), "" + mandate.getChannel(),
						mandate.getStatus().getName(), "" + mandate.getFrequency(), mandate.getDateCreated().toString(),

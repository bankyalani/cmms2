...
<script>
	var editor = new $.fn.dataTable.Editor({
		...
		ajax : function(method, url, data, successCallback, errorCallback) {
			...
			if (data.action === 'edit') {
				$.post('<c:url value="/bank/notification/email/config/update"/>', {
					"id" : id,
				...
				$.post('<c:url value="/bank/notification/email/config/delete"/>', {
					"id" : data.id[0]
				}, function(response) {
					if (response && response == 'SUCCESS') {
						successCallback({
				...
				$.post('<c:url value="/bank/notification/email/config/add"/>', {
			...
			.DataTable(
					{
						...
						ajax : {
							"type" : "POST",
							url : "<c:url value='/bank/datatable/notification/email'/>",
							"data" : function(data) {
	...
	$('#emailConfigTable a.editor_create').on(
			...
			function(e) {
				...
				editor.field('DT_RowId').hide();
				var test = new Array();
				$.get('<c:url value="/common/mandateStatuses/get?type=mail&user=bank"/>').done(
						function(data) {
							data = $.parseJSON(data);
							$.each(data, function(i, item) {
								obj = {
